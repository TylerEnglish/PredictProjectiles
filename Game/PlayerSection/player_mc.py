...
model_filename = 'mc_trained_model.pth'
...
from models.b_qnet_mc import DQN, MarkovChain
...
class Game:
    ...
    def __init__(self, physics_engine, collision_manager, player, agent, markov_chain, training_mode=True, seconds_time=60):
        pygame.init()
    ...
    def get_current_state(self):
        return self.markov_chain.current_state
    ...
    def apply_action(self, action):
        ...
        self.markov_chain.transition(current_state, action)
    ...
    def run(self, external_agent=None):
        ...
        while self.running:
            ...
            # Decision making
            state = self.markov_chain.current_state
...
class Agent:
    def __init__(self, state_size, action_size, model, markov_chain):
        self.state_size = state_size
        self.action_size = action_size
        self.model = model
        self.markov_chain = markov_chain  # New attribute for the Markov Chain
    ...
    def act(self, state, training_mode):
        if not training_mode and np.random.rand() > self.epsilon:
            # Use the Markov Chain to predict the future states
            predicted_states = self.markov_chain.predict(state)
...
def main_mc(training_mode,num_episodes):
    ...
    markov_chain = MarkovChain(state_size, action_size)

    agent = Agent(state_size, action_size, model, markov_chain)
    ...
    if training_mode:
        for i in range(0, num_episodes):
            ...
            collision_manager = CollisionManager()
            player = Player(PLAYER_INITIAL_HEALTH)
            game = Game(physics, collision_manager, player, agent, markov_chain, training_mode, 60)
            reward = game.run()
