...
from models.qnet_basic import DQN
...
class Game:
    ...
    def __init__(self, physics_engine, collision_manager, player, agent, training_mode=True, seconds_time=60):
        pygame.init()
    ...
    def apply_action(self, action):
        if action == 0:
            self.player.move(-1)  # Move left
        elif action == 1:
            self.player.move(1)  # Move right
        elif action == 2:
            self.player.move(0)  # Stay still
...
class Agent:
    ...
    def replay(self, batch_size):
        ...
        if len(self.memory) < batch_size:
            # print("Not enough samples in memory to replay.") # Test
        ...
        actions = torch.tensor(actions).unsqueeze(-1)  # Reshape for gather
        ...
        # print(f"Q targets next: {Q_targets_next}") # Test

        # Compute Q targets for current states 
        Q_targets = rewards + (0.95 * Q_targets_next * (~dones))
        # print(f"Q targets: {Q_targets}") # Test

        # Get expected Q values from current states
        Q_expected = self.model(states).gather(1, actions)
        # print(f"Q expected: {Q_expected}") #Test

        # Compute loss
        loss = F.mse_loss(Q_expected, Q_targets)
        # print(f"Loss: {loss.item()}") # Test
    ...
    if not training_mode:
        if os.path.exists(model_path):
            ...
            while not game_over:
                ...
                new_state, reward, game_over = game.apply_action(action)
